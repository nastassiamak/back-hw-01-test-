...
export function validateCreateVideoInput(input: Record<string, any>): APIErrorResult | null {
    ...
    if (input.title !== undefined && (input.title === null || typeof input.title !== 'string')) {
        errors.push({ message: "Title must be a string and cannot be null.", field: "title" });
    }

    // Проверка author
    if (input.author && (typeof input.author !== 'string' || input.author.length > 20)) {
        errors.push({ message: "Author must be a string with a maximum length of 20.", field: "author" });
    }

    // Проверка availableResolutions
    if (input.availableResolutions !== undefined) {
